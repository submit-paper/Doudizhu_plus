...
from .models import Model, Coach, Pre_model
...
def predict_loss(res, label, criterion):
...
def train(flags):  
        ...
        for i in range(flags.num_actors):
            actor = ctx.Process(
                ...
                args=(i, device, free_queue[device], full_queue[device], coach_free_queue[device], coach_full_queue[device], \
                    pre_models[device], models[device], coach_actor_models[device], buffers[device], \
                      coach_buffers[device], flags, coach_thresh, share_lock))
            actor.start()
    ...
    def batch_and_learn(i, device, position, local_lock, position_lock, lock=threading.Lock()):
        ...
        while frames < flags.total_frames:
            ...
            other_criterion = torch.nn.CrossEntropyLoss().to(device)
            _stats = learn(position, pre_models, predict_model.get_model(position), models, learner_model.get_model(position),
                           batch, optimizers[position], flags, position_lock, other_criterion)
